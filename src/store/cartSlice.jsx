...
export const fetchCartProducts = createAsyncThunk('cart/fetchCartProducts', async () => {
  try {
    ...
    const response = await fetch(`https://ecommerce-rmws.onrender.com/api/v1/cart`, {
      method: 'GET',
      headers: {
...
export const removeProductFromCart = createAsyncThunk('cart/removeProductFromCart', async (productId) => {
    try {
    const response = await fetch(`https://ecommerce-rmws.onrender.com/api/v1/cart/${productId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  async (productId) => {
    try {
      const response = await fetch(
        `https://ecommerce-rmws.onrender.com/api/v1/cart/increase/${productId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
...
export const decreaseQuantity = createAsyncThunk(
  ...
  async (productId) => {
    try {
      const response = await fetch(
        `https://ecommerce-rmws.onrender.com/api/v1/cart/decrease/${productId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
...
export const removeAllProducts = createAsyncThunk('cart/removeAllProducts',async (productId) => {
    try {
      const response = await fetch(
        `https://ecommerce-rmws.onrender.com/api/v1/cart`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',

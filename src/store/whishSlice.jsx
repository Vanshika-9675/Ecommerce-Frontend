...
export const fetchwishProducts = createAsyncThunk('wish/fetch', async () => {
  try {
      const response = await fetch('https://ecommerce-rmws.onrender.com/api/v1/wish', {
        method: 'GET',
        headers: {
...
export const deleteWishProduct = createAsyncThunk('wish/delete', async (wishId) => {
  try {
    const response = await fetch(`https://ecommerce-rmws.onrender.com/api/v1/wish/${wishId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
